...
//convert gridpoint to coordinates
/*
double** find_Coords(int Gridpoint){
    ...
    // Create a grid for 10cm, in volume 1x1m
    int step = 10;
    int gridpoint[10][10][10], z, r, c;  
    //double coords[1000][3];
    double** coords = 0;
    coords = new double*[3];
    //double vtx_z,vtx_y,vtx_x;
        ...
        for(int z=0; z<10 ; z++){ 
                for(int r=0; r<10 ; r++){ 
                        for(int c=0; c<10 ; c++){ 
                                ...
                                i++;
                            // Find vtx coordinates for the given gridpoint
                            coords[i][2] = -50 + z * step + 5;
   		            coords[i][1] = -50 + r * step + 5;		
		            coords[i][0] = -50 + c * step + 5;
                            //cout<<"i :"<<i<<coords[0][0]<<endl;
                            //cout<<"vtx_x "<<vtx_x<<" vtx_y "<<vtx_y<<" vtx_z "<<vtx_z<<endl;
                        }
                }
         }
return 1;
...
*/
/*//creates the gridpoints in a fiducial volume and returns the coordiantes for each grid point
double grid_coords
{
...
*/
void tankLocalFindTrackLengthInWaterVtxRecoNocuts()
{
   ...
   //csvfile.open ("tankPMT_forVetrexReco_final.csv");
    //csvfile.open ("tankPMT_withonlyMRDcut_insidevolume.csv");
    csvfile.open ("tankPMT_nocut_insidevolume.csv");
...
*/
   ...
   sprintf(fname,"../../../../Desktop/Evi_scripts/vtxreco-beamlikemrd.root");
